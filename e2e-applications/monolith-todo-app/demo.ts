...
function main() {
  ...
  const exampleAppPath = resolve(process.cwd(), "e2e-applications/monolith-todo-app/src");
  ...
  console.log(`📊 Discovered ${allMethods.length} endpoint methods across controllers\n`);

  // Group by controllers
  const controllerGroups = allMethods.reduce((acc, method) => {
  ...
  const sampleMethod = allMethods.find(
    m => m.controllerName === 'TodoController' && 
         m.httpMethod === 'GET' && 
         m.path === '/todos'
  );
  ...
  console.log("\n✅ E2E Demo Complete!");
  console.log(`Processed ${allMethods.length} endpoints from ${Object.keys(controllerGroups).length} controllers`);
  console.log("All methods successfully analyzed and transformed to OpenAPI format! 🎉");
}
