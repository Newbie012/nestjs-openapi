...
seoTitle: 'Migrate from @nestjs/swagger'
description: 'Step-by-step migration from @nestjs/swagger to nestjs-openapi: key differences, what to keep, which decorators become unnecessary and how to verify the output.'
---
...
If the OpenAPI routes depend on bootstrap-time config, use `OpenApiModule.setup()` in `main.ts`:
...
@Module({
  imports: [
    OpenApiModule.forRoot({
      specFile: 'openapi.json',
      swagger: { path: '/docs' }, // Different path
    }),
  ],
})
```
