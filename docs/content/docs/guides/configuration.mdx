---
title: Configuration
seoTitle: 'Configuration options'
description: 'Every option in the nestjs-openapi config file: output path and format, entry module, tsconfig, OpenAPI info, servers, security schemes and filters.'
---
...
<Callout type="note">
  ...
  inlined—they always use a schema reference regardless of this setting.
</Callout>
...
Controls how pass-through alias schemas are emitted (default: `"collapse"`).

| Value        | Description                                                                                            |
| ------------ | ------------------------------------------------------------------------------------------------------ |
| `"collapse"` | Rewrite alias chains like `A -> B -> C` to `A -> C` when `B` is only a `$ref` wrapper, then remove `B` |
