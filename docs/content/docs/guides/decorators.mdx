...
When using `@Query()` without an explicit parameter name, DTO properties are **inlined as individual query parameters** by default:
...
This is the standard OpenAPI practice and works better with code generators. If you prefer to keep the DTO as a single schema reference, set `query.style: "ref"` in your config. See the [Configuration Guide](/docs/guides/configuration#query) for details.
