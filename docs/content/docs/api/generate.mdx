...
```typescript
import { generate, formatValidationResult } from 'nestjs-openapi-static';
...
```bash
npx nestjs-openapi-static generate -c <config-path> [options]
```
...
npx nestjs-openapi-static generate -c openapi.config.ts --format yaml

# Quiet mode for CI
npx nestjs-openapi-static generate -c openapi.config.ts --quiet
